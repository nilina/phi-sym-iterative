...
void cetaflatHEP2t(int nIterN=1, double Ethr1=4, double Ethr2=150) {
  ...
  for (int ii=0;ii<14;ii++) {
    ...
    for (int ll=0;ll<72;ll++) {
      ...
      //sprintf(ctit,"HBHEInter397/eHEspec/E_+%d_%d_2",ieta,iphi);
  ...
  //TFile *histf = new TFile("HEP2mc.root","RECREATE");

  FILE *ft1;
  sprintf(ctit,"corrHEP2_%s_%d_%d.txt",ftit,((int) Ethr1),((int) Ethr2));
  //sprintf(ctit,"corrHEP2_MC_%d_%d.txt",((int) Ethr1),((int) Ethr2));
  ...
  for (int ii=0;ii<14;ii++) {
    ...
    sprintf(ctit,"HE  %d/i#phi/2",ieta); 
    tt1.DrawLatex(45,ahmax*0.933,ctit);
    sprintf(ctit,"/home/vodib/papers/calibration/figures/HEPcorr_%d_2.pdf",ieta);
    ccc[ii]->Print(ctit);
    sprintf(ctit,"/home/vodib/papers/calibration/figures/HEPcorr_%d_2.eps",ieta);
    ccc[ii]->Print(ctit);
    */
    //hprcL[ii]->Write();
    ...
    sprintf(ctit,"/home/vodib/papers/calibration/figures/HEP_E_%d_2.pdf",ieta);
    cpr[ii]->Print(ctit);
    sprintf(ctit,"/home/vodib/papers/calibration/figures/HEP_E_%d_2.eps",ieta);
    cpr[ii]->Print(ctit);
    */
    //hprL0[ii]->Write();
