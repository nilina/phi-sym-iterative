...
#include "/home/vodib/beam12/intercal/hbhe/skipHEChannel.C"

void cetaflatHEM2t(int nIterN=1, double Ethr1=4, double Ethr2=150) {
  ...
  for (int ii=0;ii<14;ii++) {
    ...
    for (int ll=0;ll<72;ll++) {
      ...
      sprintf(ctit,"phaseHF/eHEspec/E_-%d_%d_2",abs(ieta),iphi);
    ...
    for (int jj=0;jj<72;jj++) {
      ...
      if (abs(ieta)<18) continue;
      if (abs(ieta)>20 && iphi%2==0) continue;  
      if (skipHEChannel(iphi,ieta) || (ieta==-19 && iphi==43)) {
  ...
  grL->Draw("1+PAl");
  cgL->Print("pictHEplot/phiProfHEM2.gif");
  //cgL->Print("HEmc/phiProfM2.gif");


  //TFile *histf = new TFile("HEM2mc.root","RECREATE");

  FILE *ft1;
  sprintf(ctit,"corrHEM2_%s_%d_%d.txt",ftit,((int) Ethr1),((int) Ethr2));
  //sprintf(ctit,"corrHEM2_MC_%d_%d.txt",((int) Ethr1),((int) Ethr2));
  ...
  for (int ii=0;ii<14;ii++) {
    ...
    sprintf(ctit,"HEMcorr_%d_2",ieta);  // draw corrections
    ...
    hprcL[ii]->Draw("e");
    lin1->Draw();
    sprintf(ctit,"pictHEplot/HEM2corr_%d.gif",ieta);
    //sprintf(ctit,"HEmc/HEM2c_4_100G_%d.gif",ieta);
    ccc[ii]->Print(ctit);
    //hprcL[ii]->Write();

    sprintf(ctit,"HEM_E_%d_2;i#phi;GeV",ieta);
    ...
    sprintf(ctit,"pictHEplot/HEM_E_%d_2.gif",ieta);
    //sprintf(ctit,"HEmc/HEM_E_%d_2.gif",ieta);
    cpr[ii]->Print(ctit);
    //hprL0[ii]->Write();
  ...
  c1corr->Print("pictHEplot/corrHEM2.gif");
  //c1corr->Print("HEmc/corrHEM2.gif");
  //hcorr1D->Write(); 
  ...
  hmapP->SetAxisRange(hmapP->GetBinContent(11,1)/2,-1111,"Z");
  hmapP->Draw("colz");
  chmapP->Print("pictHEplot/hmapHEM2.gif");
  chmapP->Update();

  TCanvas *chmapP0 = new TCanvas("chmapP0","chmapP0",0,0,650,600);
  chmapP0->cd(); chmapP0->SetRightMargin(0.12); chmapP0->SetLogz();
  hmapP0->SetAxisRange(hmapP0->GetBinContent(11,1)/2,-1111,"Z");
  hmapP0->Draw("colz");
  chmapP0->Print("pictHEplot/hmap0HEM2.gif");
  chmapP0->Update();
  ...
  hmapPc->Draw("colz");
  chmapPc->Print("pictHEplot/hmapcHEM2.gif");
  chmapPc->Update();

  sprintf(ctit,"HEM2o_%s_%d_%d.root",ftit,((int) Ethr1),((int) Ethr2));
