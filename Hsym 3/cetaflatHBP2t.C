...
void cetaflatHBP2t(int nIterN=1, double Ethr1=4, double Ethr2=100) {
  ...
  for (int ii=14;ii<16;ii++) {
    ...
    for (int ll=0;ll<72;ll++) {
      ...
      //sprintf(ctit,"HBHEInter397/eHBspec/E_+%d_%d_2",ieta,iphi);
	...
	if (rLP>0) {
	  drLP=sqrt(pow(hdatP[ii][jj]->GetMeanError()/hdatP[ii][jj]->GetMean(),2)+
  ...
  //cgL->Print("HBmc/phiProfP2.gif");

  //TFile *histf = new TFile("HBP2mc.root","RECREATE");

  FILE *ft1;
  sprintf(ctit,"corrHBP2_%s_%d_%d.txt",ftit,((int) Ethr1),((int) Ethr2));
  //sprintf(ctit,"corrHBP2_MC_%d_%d.txt",((int) Ethr1),((int) Ethr2));
  ...
  for (int ii=14;ii<16;ii++) {
    ...
    sprintf(ctit,"HBPcorr_%d_2",ieta);  // draw corrections
    ...
    hprcL[ii]->Draw("e");
    lin1->Draw();
    sprintf(ctit,"pictHBplot/HBP2corr_%d.gif",ieta);
    //sprintf(ctit,"HBmc/HBP2c_4_100G_%d.gif",ieta);
    ccc[ii]->Print(ctit);
    //hprcL[ii]->Write();

    sprintf(ctit,"HBP_E_%d_2",ii+1);
    ...
    sprintf(ctit,"pictHBplot/HBP_E_%d_2.gif",ieta);
    //sprintf(ctit,"HBmc/HBP_E_%d_2.gif",ieta);
    cpr[ii]->Print(ctit);
    //hprL0[ii]->Write();
  ...
  c1corr->Print("pictHBplot/corrHBP2.gif");
  //c1corr->Print("HBmc/corrHBP2.gif");
  //hcorr1D->Write(); 
  ...
  hmapP->Draw("colz");
  chmapP->Print("pictHBplot/hmapHBP2.gif");

  TCanvas *chmapP0 = new TCanvas("chmapP0","chmapP0",0,0,650,600);
  chmapP0->SetRightMargin(0.12);
  hmapP0->Draw("colz");
  chmapP0->Print("pictHBplot/hmap0HBP2.gif");

  TCanvas *chmapPc = new TCanvas("chmapPc","chmapPc",0,0,650,600);
  chmapPc->SetRightMargin(0.12);
  hmapPc->Draw("colz");
  chmapPc->Print("pictHBplot/hmapcHBP2.gif");

  sprintf(ctit,"HBP2o_%s_%d_%d.root",ftit,((int) Ethr1),((int) Ethr2));
