...
void cetaflatHEP1t(int nIterN=1, double Ethr1=4, double Ethr2=150) {
  ...
  TCanvas *ccxx = new TCanvas("ccxx","ccxx",300,400,700,400);
  ccxx->Divide(2,1);
  for (int ii=0;ii<14;ii++) {
    ...
    for (int ll=0;ll<72;ll++) {
      ...
      //sprintf(ctit,"HBHEInter397/eHEspec/E_+%d_%d_1",ieta,iphi);
      sprintf(ctit,"phaseHF/eHEspec/E_+%d_%d_1",ieta,iphi);
  ...
  grL->Draw("1+PAl");
  cgL->Print("pictHEplot/phiProfHEP1.gif");


  //TFile *histf = new TFile("HEP1mc.root","RECREATE");

  FILE *ft1;
  sprintf(ctit,"corrHEP1_%s_%d_%d.txt",ftit,((int) Ethr1),((int) Ethr2));
  //sprintf(ctit,"corrHEP1_MC_%d_%d.txt",((int) Ethr1),((int) Ethr2));
  ...
  for (int ii=0;ii<14;ii++) {
    ...
    sprintf(ctit,"HEPcorr_%d_1",ieta);  // draw corrections
    ...
    hprcL[ii]->Draw("e");
    lin1->Draw();
    sprintf(ctit,"pictHEplot/HEP1corr_%d.gif",ieta);
    ccc[ii]->Print(ctit);
    /*
    ahmax=hprcL[ii]->GetMaximum();
    sprintf(ctit,"HE  %d/i#phi/1",ieta); 
    tt1.DrawLatex(45,ahmax*0.933,ctit);
    sprintf(ctit,"/home/vodib/papers/calibration/figures/HEPcorr_%d_1.pdf",ieta);
    ccc[ii]->Print(ctit);
    sprintf(ctit,"/home/vodib/papers/calibration/figures/HEPcorr_%d_1.eps",ieta);
    ccc[ii]->Print(ctit);
    */
    //hprcL[ii]->Write();

    sprintf(ctit,"HEP_E_%d_1;i#phi;GeV",ieta);
    ...
    sprintf(ctit,"pictHEplot/HEP_E_%d_1.gif",ieta);
    cpr[ii]->Print(ctit);
    /*
    ahmax=hprL0[ii]->GetMaximum();
    sprintf(ctit,"HE  %d/i#phi/1",ieta); 
    tt1.DrawLatex(45,ahmax*0.95,ctit);
    sprintf(ctit,"/home/vodib/papers/calibration/figures/HEP_E_%d_1.pdf",ieta);
    cpr[ii]->Print(ctit);
    sprintf(ctit,"/home/vodib/papers/calibration/figures/HEP_E_%d_1.eps",ieta);
    cpr[ii]->Print(ctit);
    */
    //hprL0[ii]->Write();
  ...
  c1corr->Print("pictHEplot/corrHEP1.gif");
  //hcorr1D->Write(); 
  ...
  hmapP->SetAxisRange(hmapP->GetBinContent(2,1)/2,-1111,"Z");
  hmapP->Draw("colz");
  chmapP->Print("pictHEplot/hmapHEP1.gif");
  chmapP->Update();

  TCanvas *chmapP0 = new TCanvas("chmapP0","chmapP0",0,0,650,600);
  chmapP0->cd(); chmapP0->SetRightMargin(0.12); chmapP0->SetLogz();
  hmapP0->SetAxisRange(hmapP0->GetBinContent(2,1)/2,-1111,"Z");
  hmapP0->Draw("colz");
  chmapP0->Print("pictHEplot/hmap0HEP1.gif");
  chmapP0->Update();
  ...
  hmapPc->Draw("colz");
  chmapPc->Print("pictHEplot/hmapcHEP1.gif");
  chmapPc->Update();

  sprintf(ctit,"HEP1o_%s_%d_%d.root",ftit,((int) Ethr1),((int) Ethr2));
