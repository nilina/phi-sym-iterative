...
void cetaflatHBMt(int nIterN=20, double Ethr1=4, double Ethr2=100) {
  ...
  sprintf(ctit,"yhisto_6_vtx3_1.root",ftit); 
  ...
  hmapPc->Sumw2();
  TH1F *hcorr1D = new TH1F("hcorr1D","Corr",300,0.,0.3);
  ...
  for (int ii=0;ii<16;ii++) {
    ...
    for (int ll=0;ll<72;ll++) {
      ...
      //sprintf(ctit,"HBHEInter370p4/eHBspec/E_-%d_%d_1",-ieta,iphi);
  ...
  grL->Draw("1+PAl");
  cgL->Print("pictHBplot/phiProfHBM1.gif");

  //TFile *histf = new TFile("HBM1mc.root","RECREATE");

  FILE *ft1;
  sprintf(ctit,"corrHBM1_%s_%d_%d.txt",ftit,((int) Ethr1),((int) Ethr2));
  //sprintf(ctit,"corrHBM1dec22_%d_%d.txt",((int) Ethr1),((int) Ethr2));
  ...
  for (int ii=0;ii<16;ii++) {
    ...
    hprcL[ii]->Draw("e");
    lin1->Draw();
    sprintf(ctit,"pictHBplot/HBMcorr_%d.gif",ieta);
    ccc[ii]->Print(ctit);
    /*
    ahmax=hprcL[ii]->GetMaximum();
    sprintf(ctit,"HB  %d/i#phi/1",ieta); 
    tt1.DrawLatex(45,ahmax*0.933,ctit);
    sprintf(ctit,"/home/vodib/papers/calibration/figures/HBMcorr_%d_1.pdf",ieta);
    ccc[ii]->Print(ctit);
    sprintf(ctit,"/home/vodib/papers/calibration/figures/HBMcorr_%d_1.eps",ieta);
    ccc[ii]->Print(ctit);
    */
    //hprcL[ii]->Write();
    ...
    sprintf(ctit,"pictHBplot/HBM_E_%d_1.gif",ieta);
    cpr[ii]->Print(ctit);
    /*
    ahmax=hprL0[ii]->GetMaximum();
    sprintf(ctit,"HB  %d/i#phi/1",ieta); 
    tt1.DrawLatex(45,ahmax*0.933,ctit);
    sprintf(ctit,"/home/vodib/papers/calibration/figures/HBM_E_%d_1.pdf",ieta);
    cpr[ii]->Print(ctit);
    sprintf(ctit,"/home/vodib/papers/calibration/figures/HBM_E_%d_1.eps",ieta);
    cpr[ii]->Print(ctit);
    */
    //hprL0[ii]->Write();
    ...
    for (int jj=0;jj<72;jj++) {
      ...
      //  cout<<"***************corrL = "<<corrL<<"; dcorrL = "<<dcorrL<<endl;
      if (fabs(ieta) == 14)   {
	hcorr1D->Fill(dcorrL);
	//cout<<"filling....."<<endl;
      }
  ...
  //hcorr1D->Draw("statsame");
  c1corr->Print("pictHBplot/corrHBM1.gif");
  //corr1D->Write(); 
  ...
  hmapP->Draw("colz");
  chmapP->Print("pictHBplot/hmapHBM1.gif");
  chmapP->Update();
  ...
  hmapP0->Draw("colz");
  chmapP0->Print("pictHBplot/hmap0HBM1.gif");
  chmapP0->Update();
  ...
  hmapPc->Draw("colz");
  chmapPc->Print("pictHBplot/hmapcHBM1.gif");
  chmapPc->Update();
