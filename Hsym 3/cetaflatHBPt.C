...
#include "/home/vodib/beam12/intercal/hbhe/skipHBChannel.C"

void cetaflatHBPt(int nIterN=1, double Ethr1=4, double Ethr2=100) {
  ...
  TH2F* hmapP = new TH2F("hmapP","E  HBP;i#eta;i#phi",16,0.5,16.5,72,0,72);
  TH2F* hmapP0 = new TH2F("hmapP0","E0  HBP;i#eta;i#phi",16,0.5,16.5,72,0,72);
  TH2F* hmapPc = new TH2F("hmapPc","corr  HBP;i#eta;i#phi",16,0.5,16.5,72,0,72);
  hmapPc->Sumw2();
  ...
  for (int ii=0;ii<16;ii++) {
    ...
    for (int ll=0;ll<72;ll++) {
      ...
      //sprintf(ctit,"HBHEInter397/eHBspec/E_+%d_%d_1",ieta,iphi);
	...
	hmapP->SetBinContent(ii+1,jj+1,hmapP0->GetBinContent(ii+1,jj+1,rLP));
	continue;
      }
	...
	dcorrL=(rLP-mLE)/mLE; if (fabs(dcorrL)>0.15) dcorrL=0.15*dcorrL/fabs(dcorrL);
	if (rLP>0) drLP=
	      sqrt(pow(hdatP[ii][jj]->GetMeanError()/hdatP[ii][jj]->GetMean(),2)+
		   1.f/hdatP[ii][jj]->Integral()+
		   pow(dcorrL/(2.0+sqrt((float) nIter)),2));
	else drLP=1.e-6;
  ...
  grL->SetTitle("HBP ;#eta;E / #Delta#eta ,  GeV");
  grL->Draw("1+PAl");
  cgL->Print("pictHBplot/phiProfHBP1.gif");
  //cgL->Print("HBmc/phiProfP1.gif");

  //TFile *histf = new TFile("HBP1mc.root","RECREATE");

  FILE *ft1;
  sprintf(ctit,"corrHBP1_%s_%d_%d.txt",ftit,((int) Ethr1),((int) Ethr2));
  //sprintf(ctit,"corrHBP1_MC_%d_%d.txt",((int) Ethr1),((int) Ethr2));
  ...
  for (int ii=0;ii<16;ii++) {
    ...
    sprintf(ctit,"HBPcorr_%d_1",ieta);  // draw corrections
    ...
    hprcL[ii]->Draw("e");
    lin1->Draw();
    sprintf(ctit,"pictHBplot/HBP1corr_%d.gif",ieta);
    //sprintf(ctit,"HBmc/HBP1c_4_100G_%d.gif",ieta);
    ccc[ii]->Print(ctit);
    //hprcL[ii]->Write();

    sprintf(ctit,"HBP_E_%d_1",ii+1);
    hprL0[ii] = hmapP0->ProjectionY(ctit,ii+1,ii+1);
    hprL0[ii]->SetTitle(ctit);
    sprintf(ctit,"HBP__%d",ii+1);
    ...
    sprintf(ctit,"pictHBplot/HBP_E_%d_1.gif",ieta);
    //sprintf(ctit,"HBmc/HBP_E_%d_1.gif",ieta);
    cpr[ii]->Print(ctit);
    //hprL0[ii]->Write();
  ...
  c1corr->Print("pictHBplot/corrHBP1.gif");
  //c1corr->Print("HBmc/corrHBP1.gif");
  //hcorr1D->Write(); 
  ...
  hmapP->SetAxisRange(hmapP->GetBinContent(16,1)/2,-1111,"Z");
  hmapP->Draw("colz");
  chmapP->Print("pictHBplot/hmapHBP1.gif");

  TCanvas *chmapP0 = new TCanvas("chmapP0","chmapP0",0,0,650,600);
  chmapP0->SetRightMargin(0.12);
  hmapP0->SetAxisRange(hmapP0->GetBinContent(16,1)/2,-1111,"Z");
  hmapP0->Draw("colz");
  chmapP0->Print("pictHBplot/hmap0HBP1.gif");

  TCanvas *chmapPc = new TCanvas("chmapPc","chmapPc",0,0,650,600);
  chmapPc->SetRightMargin(0.12);
  hmapPc->Draw("colz");
  chmapPc->Print("pictHBplot/hmapcHBP1.gif");

  sprintf(ctit,"HBP1o_%s_%d_%d.root",ftit,((int) Ethr1),((int) Ethr2));
