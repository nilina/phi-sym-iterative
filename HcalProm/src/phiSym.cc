...
class phiSym : public edm::EDAnalyzer {
 ...
 private:
  ...
  Int_t runNumb, EventN, NvtxEv;

  TH1F *hcounter,*herun, *hBX, *hvertex;
  TH1F *hen[26][36][2];
...
phiSym::phiSym(const edm::ParameterSet& iConfig)
{
  ...
  //  triglabel   = iConfig.getUntrackedParameter<InputTag>("triggerResultsLabel");
...
void phiSym::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   for(unsigned i=0; i<triggerNames.size(); i++) {
     ...
     cout<<" Trigger name "<<triggerNames.triggerName(i)<<std::endl;

      if(triggerResults->accept(i)) {
        cout<<"1    "<<(int)(triggerNames.triggerName(i)).find("DoubleMu7")<<endl;
	cout<<"2    "<<(int)(triggerNames.triggerName(i)).find("Mu13_Mu8")<<endl;
        cout<<"3    "<<(int)(triggerNames.triggerName(i)).find("Mu17_Mu8")<<endl ;
        cout<<"4    "<<(int)(triggerNames.triggerName(i)).find("Mu17_TkMu8")<<endl;
        cout<<"5    "<<(int)(triggerNames.triggerName(i)).find("Mu22_TkMu8")<<endl ;
        cout<<"6    "<<(int)(triggerNames.triggerName(i)).find("Mu22_TkMu22")<<endl ;
        cout<<"7    "<<	(int)(triggerNames.triggerName(i)).find("HLT_IsoMu24_eta2p1")<<endl  ;
        cout<<"8    "<<	(int)(triggerNames.triggerName(i)).find("HLT_IsoMu24")<<endl  ;
	//	if((int)(triggerNames.triggerName(i)).find("HLT_IsoMu24_eta2p1") == 0 ) { cout <<"trigger failed********************"<<endl;
	//                                                                        return;
	//	}
   ...
   */
   ...
   if (hf_hits_h.isValid()) {   
     ...
     for (HFRecHitCollection::const_iterator hfhit=hf_hits->begin(); hfhit!=hf_hits->end(); hfhit++) {
       ...
       //if (flB+flSTD<=0) 
       hen[jeta][(iphi-1)/2][depth-1]->Fill(energy);
...
void phiSym::beginJob() {
  ...
  herun = fs->make< TH1F>("herun","E(HF) vs Nrun;Nrun;GeV",11000,189000,200000);
  ...
  /// hvertex = fs->AddVariable( "NvtxEv", &NvtxEv );
